...
void FFCalculator::calcFFCorr(const Int_t mode, const TString pre_main, const std::vector<TString> pre_sub, const TString FF_file, const TString weight_file, const Int_t cuts)
{
  ...
  if ( (mode & SR) ) { // for debugging purposes calculate FF in SR and TTbar 
    doCR=0;
  } 
...
void FFCalculator::calc_nonclosure(const Int_t mode, const TString raw_ff, const TString compare_file, TString ff_output, const TString tight_cat, const Int_t doPlot, const Int_t subtractMC, const Int_t tau_ind){
  ...
  if(!subtractMC) sample+="_MC"; // happens for TTbar since it is taken from MC
  ...
  if(subtractMC){ // if not sample TTbar then this is true
    ...
    // compare_l->Rebin(1);
    // compare_l_MCsubtracted->Rebin(1);
    // compare_l_dataminusMC->Rebin(1);
    ...
    // compare_t->Rebin(1);
    // compare_t_MCsubtracted->Rebin(1);
  ...
  for(int i=0; i<g->GetN(); i++){
    ...
    else if(CHAN==kEL && i>240){ //CHANGE 110
      g->GetPoint(i,x,y);
      g->SetPoint(i,x,y200);
    }
  ...
  if(subtractMC){
    ...
    if(CHAN==kMU)g_mcup->GetPoint(240,x200,y200);
    if(CHAN==kEL)g_mcup->GetPoint(240,x200,y200); // TODO CHANGE (110*4,x200,y200)
    ...
    if(CHAN==kMU)g_mcdown->GetPoint(240,x200,y200);
    if(CHAN==kEL)g_mcdown->GetPoint(240,x200,y200); // TODO CHANGE (110*4,x200,y200)
  ...
  c2->Write();
  // from above commend to here could be deleted
...
void FFCalculator::calc_OSSScorr(const Int_t mode, const TString raw_ff, const TString SR_file_AI, TString nonclosure_corr, TString ff_output, const TString tight_cat, const Int_t doPlot, const Int_t subtractMC, const Int_t tau_ind){
  ...
  // if (CHAN == kEL || CHAN == kMU) {   //rebin 1or better statistics in et and mt channel
  //   // output_h->Rebin(1);
  //   // closure_h->Rebin(1);
  // }
  Double_t FF_value=0;
...
void FFCalculator::calc_mtcorr(const Int_t mode, const TString raw_ff, const TString CR_file, TString nonclosure_corr, TString ff_output, const TString tight_cat, const Int_t doPlot, const Int_t tau_ind){
  ...
  */
  ...
  TGraphAsymmErrors *g=   gsk.returnSmoothedGraph();
  Double_t x200; Double_t y200;
  if(CHAN==kMU)g->GetPoint(600*2./5.,x200,y200);
  if(CHAN==kEL)g->GetPoint(600*2./5.,x200,y200); // TODO CHANGE (110*4,x200,y200)
  for(int i=0; i<g->GetN(); i++){
    Double_t x; Double_t y;
    if(CHAN==kMU && i>600*2./5.){
      g->GetPoint(i,x,y);
      g->SetPoint(i,x,y200);
    }
    else if(CHAN==kEL && i>600*2./5.){ //CHANGE 110
      g->GetPoint(i,x,y);
      g->SetPoint(i,x,y200);
    }
